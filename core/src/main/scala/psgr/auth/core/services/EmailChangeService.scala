...
class EmailChangeService @Inject() (
  ...
  def finish(userId: AuthUserId, newEmail: String): Future[AuthUserId] = {
    ...
    val identity = UserIdentityModel(
      firstName = "",
      lastName = "",
      fullName = "",
  ...
  private def saveNewEmailIdentity(identity: UserIdentityModel): Future[AuthUserId] =
    for {
  ...
  private def generateHash(newEmail: String, userId: AuthUserId): String = {
    commandCryptoService.encryptCommand(ChangeEmailCommand(newEmail = newEmail, userId = userId))
  }
