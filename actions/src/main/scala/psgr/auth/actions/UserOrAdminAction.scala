...
case class UserOrAdminAction(userId: String)(implicit env: AuthEnvironment) extends UserRequiredAction {
  override def invokeBlock[A](request: Request[A], block: (UserRequiredRequest[A]) ⇒ Future[Result]): Future[Result] =
    super.invokeBlock(request, (r: UserRequiredRequest[A]) ⇒ (r.userId.id match {
      ...
      case _ ⇒
        env.permissionsService.hasPermission(r.userId, Permission.Admin)
    }).flatMap {
      case true ⇒
        block(r)
      case false ⇒
